...
export interface PartialPatch<T, U> {
     ...
     */
    parse?: (state: PartialPatchRecord<U>, snapshot: Snapshot<U>, value: T) => ParsedPartialPatch<T> | null;
}
 ...
 */
export const createPartialPatch = <T>(patch: PartialPatchRecord<T>): ParsedPartialPatch<T[keyof T]> => {
